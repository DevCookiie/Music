...
{
    public final static String NEW_VERSION_AVAILABLE = "There is a new UPDATE of DJKARL available!\n"
                    + "Current Update: %s\n"
                    + "New Version: %s\n\n";
    ...
    public static void checkVersion(Prompt prompt)
    {
        ...
        {
            prompt.alert(Prompt.Level.WARNING, "DJKARL Version", String.format(NEW_VERSION_AVAILABLE, version, latestVersion));
        }
    }
    ...
    public static String getLatestVersion()
    {
        try
        {
            Response response = new OkHttpClient.Builder().build()
                    .newCall(new Request.Builder().get().url("").build())
                    .execute();
